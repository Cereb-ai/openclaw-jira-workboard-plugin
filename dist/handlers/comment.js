...
export async function comment(args) {
    ...
    if (typeof body !== "object" || body === null || Array.isArray(body)) {
        return textResult({
            error: "comment requires `body` (ADF dict) — NOT a string. " +
    ...
    if (adf.type !== "doc" || adf.version !== 1) {
        return textResult({
            error: "comment `body` must be a valid ADF document (type='doc', version=1). " +
    ...
    if (contentErr) {
        return textResult({
            error: `comment ADF body structure error: ${contentErr}\n` +
 ...
 */
function validateMentionMap(rawMap, adf) {
    if (typeof rawMap !== "object" || rawMap === null || Array.isArray(rawMap)) {
        return ("comment `mentionMap` must be a plain object of {name: accountId}, " +
            "not an array or null. Omit the argument entirely to skip the check.");
    }
    const mentionMap = rawMap;
    ...
    for (const [name, accountId] of Object.entries(mentionMap)) {
        if (typeof name !== "string" || name.length === 0) {
            return ("comment `mentionMap` keys must be non-empty strings. " +
                `Got key of type ${typeof name}.`);
        }
        if (typeof accountId !== "string" || accountId.length === 0) {
            return (`comment \`mentionMap["${name}"]\` must be a non-empty string accountId. ` +
    ...
    const mapValues = new Set(Object.values(mentionMap));
    // ADF uses a mention whose accountId is not in the map.
    for (const id of mentionIds) {
        if (!mapValues.has(id)) {
            return (`comment: ADF body has mention node with id="${id}" but mentionMap does not contain this accountId. ` +
    ...
    for (const [name, accountId] of Object.entries(mentionMap)) {
        if (!mentionIds.has(accountId)) {
            return (`comment: mentionMap has key "${name}" → "${accountId}" but ADF body does not use this accountId. ` +
 ...
 */
function extractMentionIds(adf) {
    const out = [];
    const stack = [adf];
