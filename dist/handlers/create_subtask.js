...
export async function createSubtask(args) {
    ...
    if (typeof project !== "string" || project.trim().length === 0) {
        return textResult({
            error: "create_subtask requires `project` (string, project key, e.g. 'WTO'). " +
    ...
    if (typeof scope !== "string" || scope.trim().length === 0) {
        return textResult({
            error: "create_subtask requires `scope` (string, non-empty). " +
    ...
    if (!Array.isArray(labels) || labels.length === 0) {
        return textResult({
            error: "create_subtask requires `labels` (string[], non-empty). " +
                "Subtasks need a distinguishing tag like ['subtask:impl'] or ['subtask:test'] — there is no default.",
        });
    }
    const cleanLabels = labels.filter((l) => typeof l === "string" && l.length > 0);
