...
export async function escalateTask(args) {
    let cfg;
    try {
        cfg = loadConfig();
    }
    ...
    try {
        ...
        return textResult({
            ...
            error: `jira.escalate_task (step 3: add label) failed: ${msg}. ` +
    ...
    try {
        ...
        return textResult({
            ...
            hint: `Comment and label already applied. To finish, run: ` +
                `jira { method: "update", args: { issueIdOrKey: "${issueIdOrKey}", fields: { assignee: null } } }`,
            error: `jira.escalate_task (step 4: clear assignee) failed: ${msg}. ` +
