...
export async function requestHelp(args) {
    ...
    const mention = typeof args.mention === "string" && args.mention.length > 0
        ? args.mention
        : "";
    const mentionDisplayName = typeof args.mentionDisplayName === "string" &&
        args.mentionDisplayName.length > 0
        ? args.mentionDisplayName
        : "";
    if (mention.length > 0 && mentionDisplayName.length === 0) {
        return textResult({
            error: "request_help: when `mention` (accountId) is set, `mentionDisplayName` is also required. " +
    ...
    if (!isMainTask) {
        return textResult({
            error: `request_help 只能用于主任务。${issueIdOrKey} is a subtask — use submit_verdict({verdict:FAIL, reason}) instead.`,
        });
    }
    ...
    try {
        ...
        return textResult({
            ...
            partial: true,
            comment: { id: commentId, self: commentSelf },
            hint: `Comment was posted (id=${commentId}) but the label add failed: ${msg}. ` +
                `You can finish manually: jira { method: "add_label", args: { issueIdOrKey: "${issueIdOrKey}", label: "${WAIT_APPROVAL_LABEL}" } }`,
            error: `jira.request_help (step 3: add label) failed: ${msg}. ` +
