...
const Parameters = Type.Object({
  method: Type.Optional(
    Type.String({
      description:
        "Method name. Generic 5 (0.1.0 + 0.2.0): search, get, comment, transition, block. Orchestrator 4 (0.3.4+): create_task, create_subtask, submit_verdict, request_help. (0.3.0–0.3.3 had 5 with escalate_task; removed in 0.3.4 clean cutover.)",
    }),
  ),
  args: Type.Optional(
    Type.Record(
...
export default definePluginEntry({
  ...
  name: "Jira",
  description:
    "Native Jira Cloud REST v3 integration for OpenClaw. Single dispatcher tool `jira` covers 9 methods: generic 5 (search / get / comment / transition / block from 0.1.0 + 0.2.0) and orchestrator 4 (create_task / create_subtask / submit_verdict / request_help from 0.3.4+; escalate_task was removed in 0.3.4). submit_verdict handles both PASS and FAIL verdicts. Reads ATST_TOKEN / JIRA_CLOUD_ID from env (no mcporter dependency). Orchestrator methods also read JIRA_DEFAULT_ASSIGNEE_ACCOUNT_ID. See skills/jira/SKILL.md for per-method guidance.",
  register(api) {
